...
#Currently takes alignments for every site which is present in node 2, regardless of whether it is present in all descendants.
...
find_subst <- function(col, dS_total, dS_counter, dS_data, enhancer, startcoord) {
  ...
  if (dS_total == 0) {
    cat(myfile, file="SoxN_node2_nochange.txt", sep=" ", fill=TRUE, append=TRUE)
  }
  return(dS_data)
}  
...
files <- list.files(path="/home/sarah/utilities/play-1.2.7/sox_sites/analysis/Sequences/site_alignments/SoxN_node2", pattern=".fasta", all.files=T, full.names=T)
for (myfile in files) {
  #print(myfile)
  enhancer <- sub(".*SoxN_node2/(.*)\\.(.*)\\..*", "\\1", myfile)
  startcoord <- sub(".*SoxN_node2/(.*)\\.(.*)\\..*", "\\2", myfile)
  con <- file(myfile)
  ...
  if ((gap2 == TRUE) | (gap1 == TRUE) | (gap_mel == TRUE) | (gap_sim == TRUE) | (gap_yak == TRUE)) {
    cat(myfile, file="all_SoxN_node2_gaps.txt", sep=" ", fill=TRUE, append=TRUE)
    cat("Gaps detected", file="all_SoxN_node2_gaps.txt", sep=" ", fill=TRUE, append=TRUE)
    close(con)
  } 
  else if ((n2 == TRUE) | (n1 == TRUE) | (n_mel == TRUE) | (n_sim == TRUE) | (n_yak == TRUE)) {
    cat(myfile, file="all_SoxN_node2_n.txt", sep=" ", fill=TRUE, append=TRUE)
    cat("N detected", file="all_SoxN_node2_n.txt", sep=" ", fill=TRUE, append=TRUE)
    close(con)
  }
    ...
    node2 <- DNAString(alignment[3])
    node1 <- DNAString(alignment[5])
    mel <- DNAString(alignment[7])
    sim <- DNAString(alignment[9])
    yak <- DNAString(alignment[11])
